...
func TestScaleRect(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			r:     Rect{100, 100, 200, 200},
			scale: 0.000001,
			want:  Rect{0, 0, 0, 0}, // Due to int truncation
		},
		{
			name:  "very large scale",
			r:     Rect{100, 100, 200, 200},
			scale: 1000000.0,
			want:  Rect{100000000, 100000000, 200000000, 200000000},
		},
		{
			name:  "scale invalid rect",
			r:     Rect{200, 200, 100, 100},
			scale: 2.0,
			want:  Rect{400, 400, 200, 200}, // Still scales but maintains invalid state
		},
		{
			name:  "scale zero width/height rect",
			r:     Rect{100, 100, 100, 100},
			scale: 2.0,
			want:  Rect{200, 200, 200, 200},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := scaleRect(tt.r, tt.scale)
			if got != tt.want {
				t.Errorf("scaleRect(%v, %v) = %v, want %v", tt.r, tt.scale, got, tt.want)
			}
		})
	}
}
